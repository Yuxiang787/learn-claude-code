...
@traced_model_call(
    get_langfuse_client,
    model=MODEL,
    span_name_fn=lambda messages, sys_prompt, tools, name: f"s09-teammate:{name}",
...
@traced_model_call(
    get_langfuse_client,
    model=MODEL,
    span_name_fn=lambda messages: "s09-lead-turn",
    input_fn=lambda messages: messages,
    metadata_fn=lambda messages: {"agent": "s09_lead"},
)
